...
void	my_mlx_pixel_put(t_data *data, int x, int y, int color)
{
	...
	dst = data->addr + ((int)y
			* data->line_length + (int)x * (data->bits_per_pixel / 8));
	*(unsigned int *)dst = color;
}

/* Function that draws a line between two points. How ? Still trying to
 * figure out :D. Dont listen to my previous me. I figured it out. 
 * The file is even called Bresenham.c . Stands for the algo used. */
